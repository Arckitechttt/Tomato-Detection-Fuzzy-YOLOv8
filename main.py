...
class YoloPredictor(BasePredictor, QObject):
    ...
    # Show detected label
    def display_detected_labels(self):
        print("Detected labels:")
        for label, count in self.labels_dict.items():
            print(f"{label}: {count}")
            self.yolo2main_status_msg.emit(f"Detection completed: {label}")
        ...
        try:
            ...
            while True:
                ...
                if count + 1 >= all_count:
                    ...
                    self.display_detected_labels()  # Display detected labels
                    break
    ...
    def fuzzy_preprocessing(self, img):
        # Define fuzzy variables
        pixel = ctrl.Antecedent(np.arange(0, 641, 1), "pixel")
        grayscale = ctrl.Antecedent(np.arange(0, 256, 1), "grayscale")

        # Define fuzzy output variables
        image_quality = ctrl.Consequent(np.arange(0, 256, 1), "image_quality")

        # Define fuzzy membership functions for input variables
        pixel["low"] = fuzz.trimf(pixel.universe, [0, 160, 320])
        pixel["medium"] = fuzz.trimf(pixel.universe, [240, 320, 400])
        pixel["high"] = fuzz.trimf(pixel.universe, [320, 480, 640])

        grayscale["low"] = fuzz.trimf(grayscale.universe, [0, 64, 128])
        grayscale["medium"] = fuzz.trimf(grayscale.universe, [96, 128, 160])
        grayscale["high"] = fuzz.trimf(grayscale.universe, [128, 192, 255])

        # Define fuzzy membership functions for output variable
        image_quality["bad"] = fuzz.trimf(image_quality.universe, [0, 64, 128])
        image_quality["good"] = fuzz.trimf(image_quality.universe, [128, 192, 255])

        # Define fuzzy rules
        rule1 = ctrl.Rule(pixel["high"] & grayscale["high"], image_quality["good"])
        rule2 = ctrl.Rule(pixel["high"] & grayscale["medium"], image_quality["good"])
        rule3 = ctrl.Rule(pixel["high"] & grayscale["low"], image_quality["bad"])
        rule4 = ctrl.Rule(pixel["medium"] & grayscale["high"], image_quality["good"])
        rule5 = ctrl.Rule(pixel["medium"] & grayscale["medium"], image_quality["good"])
        rule6 = ctrl.Rule(pixel["medium"] & grayscale["low"], image_quality["bad"])
        rule7 = ctrl.Rule(pixel["low"] & grayscale["high"], image_quality["bad"])
        rule8 = ctrl.Rule(pixel["low"] & grayscale["medium"], image_quality["bad"])
        rule9 = ctrl.Rule(pixel["low"] & grayscale["low"], image_quality["bad"])

        # Create control systems
        image_quality_ctrl = ctrl.ControlSystem(
            [rule1, rule2, rule3, rule4, rule5, rule6, rule7, rule8, rule9]
        )
        image_quality_sim = ctrl.ControlSystemSimulation(image_quality_ctrl)

        if img is None:
            raise ValueError("The image is empty. Make sure the image path is correct.")
        ...
        # Compute the average pixel and grayscale values
        avg_pixel = np.mean(img)
        avg_grayscale = np.mean(gray)

        # Simulate the image quality control system
        image_quality_sim.input["pixel"] = avg_pixel
        image_quality_sim.input["grayscale"] = avg_grayscale
        image_quality_sim.compute()
        image_quality_value = image_quality_sim.output["image_quality"]

        return image_quality_value

    def preprocess_image(self, img):
        # Perform fuzzy preprocessing
        image_quality_value = self.fuzzy_preprocessing(img)

        # Transpose image to (height, width, channels) if necessary
        if img.shape[0] == 3:
            img = img.transpose(1, 2, 0)

        # Check the shape of the image before resizing
        print(f"Original image shape: {img.shape}")

        # Resize image to 320x320 using OpenCV
        img = cv2.resize(img, (320, 320), interpolation=cv2.INTER_LINEAR)

        # Check the shape of the image after resizing
        print(f"Resized image shape: {img.shape}")

        # Convert the numpy array to a torch tensor and move to the appropriate device
        img = torch.from_numpy(img).to(self.model.device)

        # Convert to float16 if model uses fp16, otherwise to float32
        img = img.half() if self.model.fp16 else img.float()  # uint8 to fp16/32

        # Normalize the image to range [0.0, 1.0]
        img /= 255.0  # 0 - 255 to 0.0 - 1.0

        # Permute the dimensions of the tensor to (batch_size, num_channels, height, width)
        img = img.permute(2, 0, 1).unsqueeze(0)

        # Optionally, you can use image_quality_value for further processing or debugging
        print(f"Image quality value: {image_quality_value}")
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Show module shadows
        UIFuncitons.shadow_style(self, self.Class_QF, QColor(0, 0, 0))
        UIFuncitons.shadow_style(self, self.Target_QF, QColor(0, 0, 0))
        # UIFuncitons.shadow_style(self, self.Fps_QF, QColor(0, 0, 0))
        UIFuncitons.shadow_style(self, self.Model_QF, QColor(0, 0, 0))
    ...
    def run_or_continue(self):
        if self.yolo_predict.source == "":
            self.show_status(
                "Please select a video source before starting the detection..."
            )
    ...
    def show_status(self, msg):
        ...
        if re.search("Detection completed", msg) or msg == "检测完成":
        ...
        elif re.search("Detection terminated!", msg) or msg == "检测终止":
            self.save_res_button.setEnabled(True)
            self.save_txt_button.setEnabled(True)
            self.run_button.setChecked(False)
            self.progress_bar.setValue(0)
    ...
    def load_config(self):
        ...
        self.run_button.setChecked(False)
        self.show_status("Hi, welcome to TomaTect App~")
    ...
    def change_val(self, x, flag):
        if flag == "iou_spinbox":
            ...
            self.show_status("Confidence Score Threshold: %s" % str(x / 100))
    ...
    def closeEvent(self, event):
        ...
        if self.yolo_thread.isRunning():
            ...
            MessageBox(
                self.close_button,
                title="Note",
                text="Exiting TomaTect App, please wait...",
                time=3000,
                auto=True,
            ).exec()
            sys.exit(0)
